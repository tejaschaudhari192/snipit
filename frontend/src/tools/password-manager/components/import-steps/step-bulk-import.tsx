...
import { CheckCircle2, CircleDashed, FileX2, AlertTriangle, SkipForward, CopyPlus, Replace } from "lucide-react";
import {
	Dialog,
	DialogContent,
	DialogHeader,
			...
			<DialogContent className="sm:max-w-[420px]" onInteractOutside={(e) => e.preventDefault()}>
				<DialogHeader>
					<DialogTitle className="flex items-center gap-2">
						...
						{t("password_manager_import_duplicate_detected")}
					</DialogTitle>
					<DialogDescription>
						<span className="font-semibold text-foreground">{pending.item.mapped.title}</span>
						{" "}
						{t("password_manager_import_duplicate_info")}
					</DialogDescription>
				</DialogHeader>
				<DialogFooter className="flex-col sm:flex-row gap-2 mt-2">
					...
					>
						...
						{t("password_manager_import_skip")}
					</Button>
					<Button
						variant="outline"
						className="flex-1 gap-2"
					...
					>
						...
						{t("password_manager_import_keep_both")}
					</Button>
					<Button
						className="flex-1 gap-2"
					...
					>
						...
						{t("password_manager_import_overwrite")}
					</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
...
// ─── Main component ──────────────────────────────────────────────────────────
export default function StepBulkImport({ items, duplicateStrategy, onDone }: Props) {
	const { t } = useTranslation();
	...
	const askForDuplicate = useCallback((item: ParsedImportItem): Promise<DuplicateDecision> => {
	...
	useEffect(() => {
		...
		const runImport = async () => {
			for (let i = 0; i < items.length; i++) {
				...
				// ── Handle duplicates ────────────────────────────────────
				if (item.isDuplicate) {
					let decision: DuplicateDecision = duplicateStrategy as DuplicateDecision;
				...
				if (item.sourceFolder) {
					...
					if (localFolderCache[item.sourceFolder]) {
						item.mapped.folderId = localFolderCache[item.sourceFolder];
					}
				}
	...
	const progress = items.length === 0 ? 100 : Math.round((totalProcessed / items.length) * 100);
	...
	return (
		<>
			...
			<div className="p-6 flex flex-col gap-8 text-center min-h-75 justify-center">
				<div>
					<h2 className="text-xl font-bold mb-2">{t("password_manager_import_importing")}</h2>
					<p className="text-muted-foreground">
						{t("password_manager_import_processing", { current: totalProcessed, total: items.length })}
					</p>
				</div>
				...
				{currentItem && (
					<div className="p-3 bg-muted/50 rounded-lg text-sm flex items-center gap-3 text-left">
						...
						<div className="truncate flex-1">
							<span className="font-medium mr-2">{t("password_manager_import_processing_item")}:</span>
						...
						{currentItem.isDuplicate && (
							<span className="text-xs font-semibold text-amber-600 bg-amber-500/10 px-2 py-0.5 rounded-full shrink-0">
								{t("password_manager_import_duplicate")}
							</span>
						)}
					</div>
				)}
				...
				<div className="pt-8">
					<Button
						...
						onClick={() => {
							...
							onDone(importedCount, skippedCount + (items.length - currentIndex));
						}}
					>
						{t("password_manager_import_stop")}
					</Button>
				</div>
			</div>
		</>
