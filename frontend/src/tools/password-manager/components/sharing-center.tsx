...
export default function SharingCenter() {
	...
	const renderCollectionCard = (coll: SharedCollection, isOwner: boolean) => (
		<div
			...
			className="p-4 border border-white/5 rounded-xl bg-black/20 hover:bg-black/40 transition-colors flex items-center justify-between"
		>
			<div className="flex items-center gap-4">
				...
				<div>
					...
					<p className="text-xs text-muted-foreground mt-1">
						{coll.items.length} item
						{coll.items.length !== 1 ? "s" : ""} • Role:{" "}
					...
					{sharedWithMe.length === 0 ? (
						<div className="flex flex-col items-center justify-center p-12 border border-white/5 rounded-xl bg-black/20 text-center">
							...
							<p className="text-sm text-muted-foreground mt-2 max-w-sm">
								When someone shares a password with you using
								End-to-End Encryption, it will appear here.
							</p>
						</div>
					) : (
				...
				<TabsContent value="by-me" className="mt-6 space-y-4">
					{sharedByMe.length === 0 ? (
						<div className="flex flex-col items-center justify-center p-12 border border-white/5 rounded-xl bg-black/20 text-center">
							...
							<p className="text-sm text-muted-foreground mt-2 max-w-sm">
								Share items individually from their details
								page, or create a collection here.
							</p>
						</div>
					) : (
						<div className="grid gap-3">
